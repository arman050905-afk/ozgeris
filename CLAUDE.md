...
**Жеткізу жиілігі — еркін, минутпен, GitHub Actions арқылы (2026-09-25)**: Vercel Hobby
жоспарында Cron Jobs күніне 1 реттен жиі жүрмейді (Pro жоспарға көшпей нағыз минуттік server cron
мүмкін емес), сол шектеуді айналып өту үшін `.github/workflows/push-reminders.yml` **әр 15 минут
сайын** (`*/15 * * * *`, + қолмен іске қосу үшін `workflow_dispatch`) `GET /api/cron/check-alerts`-ты
...
workflow" бас керек. Сол себепті нақты минуттік дәлдік кепілдендірілмеген — ~15 минутқа дейін
ауытқуы мүмкін.
...
- **Хабарландырулар** — `renderPushSection()` енді ЕКІ контейнерге де рендерлейді (Қаржы→Міндетті
  ...
  сипаттамасы жоғарыдағы «Push ескертулер» бөлімінде (GitHub Actions 15-минуттық workflow,
